...
router.post('/', async (req, res) => {
    await Chat_roomsService.create(req.body.data, req.currentUser, true, req.headers.referer);
    const payload = true;
...
router.get('/', wrapAsync(async (req, res) => {
  const payload = await Chat_roomsDBApi.findAll(
    req.query,
  );
...
router.get('/:id', wrapAsync(async (req, res) => {
  const payload = await Chat_roomsDBApi.findBy(
    { id: req.params.id },
  );
