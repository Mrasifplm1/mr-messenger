...
import * as dataFormat from 'pages/CRUD/Chat_rooms/table/Chat_roomsDataFormatters';


import actions from 'actions/chat_rooms/chat_roomsListActions';
...
const Chat_roomsTable = () => {
  ...
  const loading = useSelector((store) => store.chat_rooms.list.loading);
  const rows = useSelector((store) => store.chat_rooms.list.rows);
  const count = useSelector((store) => store.chat_rooms.list.count);
  const modalOpen = useSelector((store) => store.chat_rooms.list.modalOpen);
  const idToDelete = useSelector((store) => store.chat_rooms.list.idToDelete);
  ...
  const actionFormatter = (cell) => {
    return (
        <div>
          {null && (
          <Button color="default" size="xs" onClick={() => dispatch(push(`/admin/chat_rooms/${cell}`))}>
          View
        </Button>
        )}
        <Button color="info" size="xs" onClick={() => history.push(`/admin/chat_rooms/${cell}/edit`)}>
        Edit
      </Button>
        &nbsp;&nbsp;
  ...
  return (
  <div>
  <Widget title={<h4>Chat_rooms</h4>} collapse close>
  <Link to="/admin/chat_rooms/new">
    <button
...
export default Chat_roomsTable;
