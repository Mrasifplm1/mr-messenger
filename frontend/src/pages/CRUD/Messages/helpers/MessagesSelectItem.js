...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/messages/autocomplete`, { params });
  return response.data;
}

const MessagesSelectItem = (props) => {
  ...
  const mapper = {
    intoSelect(originalValue) {
      ...
      let label = originalValue.label ? originalValue.label : originalValue.text;
...
const select = (state) => ({
  hasPermissionToCreate: state.messages.hasPermissionToCreate
});
