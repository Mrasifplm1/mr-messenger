...
import * as dataFormat from 'pages/CRUD/Messages/table/MessagesDataFormatters';

// eslint-disable-next-line
import * as usersDataFormat from 'pages/CRUD/Users/table/UsersDataFormatters';
// eslint-disable-next-line
import * as chat_roomsDataFormat from 'pages/CRUD/Chat_rooms/table/Chat_roomsDataFormatters';

import actions from 'actions/messages/messagesListActions';
...
const MessagesTable = () => {
  const [filters, setFilters] = React.useState([
    {label: 'Text', title: 'text'},{label: 'From_user', title: 'from_user'},{label: 'Chat_room', title: 'chat_room'},{label: 'To_user', title: 'to_user'},
  ]);
  ...
  const loading = useSelector((store) => store.messages.list.loading);
  const rows = useSelector((store) => store.messages.list.rows);
  const count = useSelector((store) => store.messages.list.count);
  const modalOpen = useSelector((store) => store.messages.list.modalOpen);
  const idToDelete = useSelector((store) => store.messages.list.idToDelete);
  ...
  const actionFormatter = (cell) => {
    return (
        <div>
          {null && (
          <Button color="default" size="xs" onClick={() => dispatch(push(`/admin/messages/${cell}`))}>
          View
        </Button>
        )}
        <Button color="info" size="xs" onClick={() => history.push(`/admin/messages/${cell}/edit`)}>
        Edit
      </Button>
        &nbsp;&nbsp;
  ...
  const columns = [
    ...
    sort: true,
      ...
      formatter: chat_roomsDataFormat.listFormatter,
  ...
  return (
  <div>
  <Widget title={<h4>Messages</h4>} collapse close>
