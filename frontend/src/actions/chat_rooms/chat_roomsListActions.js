...
async function list(filter) {
  const response = await axios.get(
    `/chat_rooms?page=${filter.page}&limit=${filter.limit}&chat_rooms=${filter.chat_rooms ? filter.chat_rooms : ''}`,
  );
  return response.data;
}

async function filterChat_rooms(request, filter) {
  const response = await axios.get(`/chat_rooms?page=${filter.page}&limit=${filter.limit}${request}`);
  return response.data;
}
